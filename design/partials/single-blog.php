<?php
/**
 * Single blog post template. Expects $slug to be set by the router.
 */
...
if (!$post) {
  ...
  $pageTitle = 'Post Not Found | Suave Creators';
  $useHeroBackground = true;
  require __DIR__ . '/../layout/start.php';
  echo '<section class="site-container py-24 text-center text-white"><h1 class="text-3xl font-bold">Blog post not found</h1><p class="mt-4 text-[#B1B9DF]">The article you are looking for may have been moved or removed.</p><a href="/blogs" class="mt-8 inline-flex u-btn-cta rounded-full bg-gradient-to-r from-[#2A4DFB] to-[#0026E3] px-5 py-2 text-sm font-bold text-white">Back to Blogs</a></section>';
...
$featureImageHtml = '<figure class="single-blog-main__image single-blog-main__image--inline">'
  . '<img src="' . $h($post['image']) . '" alt="' . $h($post['title']) . '" width="1200" height="640" loading="eager">'
  . '</figure>';
$pullQuote = trim((string) ($post['short_description'] ?? ''));
if ($pullQuote !== '' && stripos($articleContent, '<blockquote') === false) {
  $quoteHtml = '<blockquote><p>' . $h($pullQuote) . '</p></blockquote>';
  $closePos = strpos($articleContent, '</p>');
  if ($closePos !== false) {
    $articleContent = substr_replace($articleContent, '</p>' . $quoteHtml . $featureImageHtml, $closePos, 4);
  } else {
    $articleContent = $featureImageHtml . $articleContent;
  }
} elseif (preg_match('/<\/blockquote>/i', $articleContent)) {
  $articleContent = preg_replace('/<\/blockquote>/i', '</blockquote>' . $featureImageHtml, $articleContent, 1);
} else {
  $closePos = strpos($articleContent, '</p>');
  if ($closePos !== false) {
    $articleContent = substr_replace($articleContent, '</p>' . $featureImageHtml, $closePos, 4);
  } else {
    $articleContent = $featureImageHtml . $articleContent;
  }
}
...
$pageTitle = $post['title'] . ' | Suave Creators Blog';
...
<section class="single-blog-top relative z-10 w-full pb-10 pt-6 md:pb-14 md:pt-8 lg:pb-16 site-container">
  <nav class="blog-breadcrumb" aria-label="Breadcrumb">
    ...
    <span aria-hidden="true">/</span>
    <span aria-current="page"><?= $h($post['title']) ?></span>
  </nav>
  ...
  <h1 class="single-blog-main__title">
    <?php if ($titleLead !== ''): ?>
      <?= $h($titleLead) ?>
    <?php endif; ?>
    <span class="single-blog-main__title-accent"><?= $h($titleAccent) ?></span>
  </h1>
        ...
        <div class="single-blog-footer">
          <ul class="single-blog-tags" aria-label="Tags">
            <?php foreach ($tags as $tag): ?>
              <li><span class="single-blog-tag"><?= $h($tag) ?></span></li>
            <?php endforeach; ?>
          </ul>
        </div>
                  ...
                  <div class="swiper-slide">
                    <article class="articles-card blog-sidebar-card">
                      <figure class="articles-card__image">
                        <img src="<?= $h($item['image']) ?>" alt="<?= $h($item['title']) ?>" width="640" height="420" loading="lazy">
                      </figure>
                      <div class="articles-card__body">
                        <div class="articles-card__meta">
                          ...
                          <time datetime="<?= $h($item['published_date']) ?>">
                            ...
                            <?= $h($item['published_label']) ?>
                          </time>
                        </div>
                        <h3><?= $h($item['title']) ?></h3>
                        <p><?= $h($item['short_description']) ?></p>
                        <a class="articles-card__link underline mt-2 text-sm font-semibold text-[#2A4DFB]" href="/blog/<?= $h($item['slug']) ?>">Read More</a>
                      </div>
                    </article>
                  </div>
                <?php endforeach; ?>
...
<!-- FAQ Section Start -->
<section class="full-bleed faq-section faq-section--align faq-section--contact bg-[url('/images/web-bg.png')] bg-cover bg-center" aria-labelledby="blog-faq-heading">
  <div class="faq-section__inner section-inner">
    <div class="faq-section__intro">
      ...
      <h2 id="blog-faq-heading">Frequently Ask Question</h2>
      <p class="faq-section__description">Here are the most asked questions based on feedback from our readers.</p>
      <?php
        $faqCtaHref = '/contact-us/#contact-id';
        $faqCtaLabel = 'Book a Consultation';
        require __DIR__ . '/faq-cta-button.php';
      ?>
      <img class="faq-section__image" src="/images/faq-gif.gif" alt="Business team collaborating around a table" width="640" height="960" loading="lazy">
    </div>

    <div class="faq-list">
      <?php foreach ($blogFaqs as $index => $faq): ?>
        <?php $faqNumber = $index + 1; ?>
        <div class="faq-item<?= $index === 0 ? ' is-open' : '' ?>">
          <button type="button" class="faq-item__summary" aria-expanded="<?= $index === 0 ? 'true' : 'false' ?>" aria-controls="blog-faq-answer-<?= $faqNumber ?>" id="blog-faq-question-<?= $faqNumber ?>">
            <span><?= $h($faq['question'] ?? '') ?></span>
            <i class="fa-solid fa-chevron-down faq-item__chevron" aria-hidden="true"></i>
          </button>
          <div class="faq-item__answer" id="blog-faq-answer-<?= $faqNumber ?>" role="region" aria-labelledby="blog-faq-question-<?= $faqNumber ?>" aria-hidden="<?= $index === 0 ? 'false' : 'true' ?>">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var faqItems = document.querySelectorAll('.faq-section .faq-list .faq-item');
