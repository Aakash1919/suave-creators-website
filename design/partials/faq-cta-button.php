<?php
$faqCtaHref = $faqCtaHref ?? '/contact-us/#contact-id';
$faqCtaLabel = $faqCtaLabel ?? 'Start your Project';
?>
<a href="<?= htmlspecialchars((string) $faqCtaHref, ENT_QUOTES, 'UTF-8') ?>"
  class="faq-section__cta group inline-flex w-fit items-center gap-2 rounded-full bg-gradient-to-r from-[#2A4DFB] to-[#0026E3] px-5 py-2 text-sm font-bold text-white shadow-lg shadow-indigo-950/30 transition hover:brightness-110">
  <?= htmlspecialchars((string) $faqCtaLabel, ENT_QUOTES, 'UTF-8') ?>
  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="14" viewBox="0 0 24 24" fill="none"
    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
    class="transition-transform duration-300 group-hover:translate-x-1" aria-hidden="true">
    <path d="M18 8L22 12L18 16"></path>
    <path d="M2 12H22"></path>
  </svg>
</a>
<?php
unset($faqCtaHref, $faqCtaLabel);
?>
