...
| Command | Purpose |
|---------|---------|
| `run-once:sanitize-blog` | Sanitize blog `content` via `BlogHtmlSupport` (same path as admin save): extract `data:image/…` and oversized inline SVG to `storage/app/public/blogs/content/{slug}-{n}.webp` (fallback original ext), fill empty `img` `alt`s, add width/height/lazy when the file is local, remove empty tags, unwrap nested headings, drop a duplicate lead title, promote body H3→H2 when the article has no H2, and print a table of sanitized blog URLs |
...
## AI trend drafts (manual; schedule commented out)
...
Schedule in `routes/console.php` (Tue/Fri at `BLOG_TREND_DRAFTS_TIME`) is **commented out** by default — uncomment to re-enable automatic runs. Manual `generate:blog` and admin “Generate blog” still work.
