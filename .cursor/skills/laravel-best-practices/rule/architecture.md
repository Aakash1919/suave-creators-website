...
CRM: use **Services** (`create-service`), not Action classes, unless the module already has them. Constructor-inject the module Service. New mutation slice = new `{Concern}Service` unless the user asked to merge. See [`crm.md`](crm.md).

## Extract Focused Business Operations

Extract a discrete business operation into a `{Concern}Service` when that makes the operation easier to reuse or test. Do not introduce Action classes as a second pattern.
