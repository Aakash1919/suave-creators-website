...
CRM: [`crm.md`](crm.md) + `create-model`. Local scopes are `scopeActive(Builder $query): Builder` matching siblings — do not add `#[Scope]` as a second pattern. `HasUuid` for uuid. Listings go to Query classes.
...
Reusable local scope (CRM form — see [`crm.md`](crm.md)):
