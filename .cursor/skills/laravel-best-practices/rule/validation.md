...
CRM: always a Form Request (`create-form-request`). **Never** `$request->validate()` in a controller, even for a small rule set. `authorize()` is typically `true` because permission middleware already ran.

## Extract Validation When It Improves the Boundary

Use a Form Request for store/update. Do not inline `$request->validate()` in CRM controllers.
