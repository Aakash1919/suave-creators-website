...
CRM: module folders `database/migrations/modules/{slug}/` registered in `TenantHelper::migrationPaths()`. Guard with `Schema::hasTable` / `hasColumn`. Every first-class table gets `uuid`. See `create-migration`.
