...
@section('content')
  <div class="admin-page-head">
    ...
    <div class="admin-page-head__actions">
      ...
      @if ($blog->exists)
        <a href="{{ route('blog.show', $blog->slug) }}" target="_blank" rel="noopener" class="admin-btn admin-btn--secondary">
  ...
  <form method="POST"
    ...
    <div class="admin-blog-form__layout">
      <div class="admin-blog-form__main">
        <section class="admin-card admin-blog-form__composer">
          <div class="admin-card__body space-y-5">
            <div>
              ...
              <input id="blog-title" type="text" name="title" value="{{ old('title', $blog->title) }}" required class="admin-input admin-input--lg" placeholder="Give this post a clear headline">
            </div>

            <div>
              <label class="admin-label" for="blog-short-description">Short description</label>
              <textarea id="blog-short-description" name="short_description" rows="3" class="admin-textarea" placeholder="One or two sentences for cards and previews">{{ old('short_description', $blog->short_description) }}</textarea>
            </div>
        ...
        <details class="admin-card admin-blog-form__seo">
          ...
          <div class="admin-card__body space-y-5">
            <div class="admin-form-grid admin-form-grid--2">
              <div>
                ...
                <input id="blog-meta-title" type="text" name="meta_title" value="{{ old('meta_title', $blog->meta_title) }}" class="admin-input">
              </div>
              <div>
                <label class="admin-label" for="blog-og-title">OG title</label>
                <input id="blog-og-title" type="text" name="og_title" value="{{ old('og_title', $blog->og_title) }}" class="admin-input">
              </div>
              <div>
              ...
              <div>
                <label class="admin-label" for="blog-toc-json">TOC JSON</label>
                <textarea id="blog-toc-json" name="toc_json" rows="6" class="admin-textarea font-mono text-xs">{{ old('toc_json', $blog->toc ? json_encode($blog->toc, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) : '') }}</textarea>
              </div>
              <div>
                <label class="admin-label" for="blog-faqs-json">FAQs JSON</label>
                <textarea id="blog-faqs-json" name="faqs_json" rows="6" class="admin-textarea font-mono text-xs">{{ old('faqs_json', $blog->faqs ? json_encode($blog->faqs, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) : '') }}</textarea>
              </div>
            </div>
          </div>
        </details>
        ...
        <section class="admin-card admin-blog-form__side-card">
          ...
          <div class="admin-card__body space-y-4">
            ...
            <div>
              ...
              <input id="blog-slug" type="text" name="slug" value="{{ old('slug', $blog->slug) }}" class="admin-input" placeholder="Auto from title if empty">
            </div>
          </div>
        </section>
          ...
          <div class="admin-card__body">
            <label class="admin-blog-form__image" for="blog-featured-image">
              @if ($blog->featuredImageUrl())
                <img src="{{ $blog->featuredImageUrl() }}" alt="Current featured image" title="Current featured image" class="admin-blog-form__image-preview">
              @else
              ...
              @endif
              <input id="blog-featured-image" type="file" name="featured_image" accept="image/*" class="admin-blog-form__image-input">
            </label>
          </div>
        </section>
