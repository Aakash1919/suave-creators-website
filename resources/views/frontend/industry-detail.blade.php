...
@section('content')
                ...
                <div class="grid grid-cols-1 gap-3 min-[480px]:grid-cols-2 sm:gap-3.5">
                    @foreach ($introStats as $stat)
                        <article
                            ...
                            <span
                                class="inline-flex h-[44px] w-[44px] shrink-0 items-center justify-center sm:h-[52px] sm:w-[52px]">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-[12px] font-medium leading-4 text-[#171717] sm:text-[13px]">
                                    {{ $stat[2] }}</p>
                            </div>
                        </article>
                    @endforeach
            ...
            <header class="web-services__header">
                <div class="mb-3 flex items-center gap-2 sm:mb-4">
                    <span
                        class="inline-block h-[16px] w-[2px] rounded-full bg-gradient-to-b from-[#2A4DFB] to-[#7A5FF8]"></span>
                    <span
                        class="inline-block bg-gradient-to-r from-[#2A4DFB] to-[#7A5FF8] bg-clip-text text-[13px] font-bold text-transparent sm:text-[14px]">{{ $industry['servicesEyebrow'] ?? 'Services' }}</span>
                </div>
                    ...
                    <article class="web-service-card">
                        <span class="web-service-card__icon">
                            <img src="{{ $service['icon'] ?? '' }}"
                                ...
                                title="{{ $service['title'] ?? 'Industry service' }} icon for Suave Creators software development"
                                width="36" height="36">
                        </span>
    ...
    <x-frontend.connect-cta-section :eyebrow="$industry['ctaEyebrow'] ?? 'Ready to Start Your Project?'" :title="$industry['ctaTitle'] ?? ''" :description="$industry['ctaDescription'] ?? ''" title-id="industry-cta-heading"
        ...
        <div class="section-inner">
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4 lg:gap-6">
                @foreach ($industry['specialized'] ?? [] as $item)
                    <article
                        class="flex min-h-full flex-col gap-3 rounded-[22px] border border-[rgba(42,77,251,0.08)] bg-white p-[22px] shadow-[0_18px_40px_rgba(36,36,84,0.06)]">
                        @if (!empty($item['icon']))
                            <span
                                class="inline-flex h-[52px] w-[52px] items-center justify-center rounded-[14px] bg-[#EEF1FF]"><img
                                    src="{{ $item['icon'] }}"
                                    alt="{{ ($item['title'] ?? 'Specialized service') . ' icon for Suave Creators industry solutions' }}"
                                    title="{{ ($item['title'] ?? 'Specialized service') . ' icon for Suave Creators industry solutions' }}"
            ...
            @for ($g = 0; $g < 2; $g++)
                <div class="digital-services-marquee__group" {{ $g === 1 ? 'aria-hidden="true"' : '' }}>
                    @foreach ($marqueeLabels as $i => $label)
                        ...
                        @endphp
                        <span
                            class="digital-services-marquee__label digital-services-marquee__label--{{ $style }}">{{ $label }}</span>
                        <span
                            class="digital-services-marquee__separator digital-services-marquee__separator--{{ $style }}"
                    ...
                    <article
                        ...
                        @if (!empty($card['icon']))
                            <span
                                class="inline-flex h-[52px] w-[52px] items-center justify-center rounded-[14px] bg-[#EEF1FF]"><img
                                    src="{{ $card['icon'] }}"
                                    alt="{{ $card['title'] ?? 'Why choose Suave Creators' }} icon"
                ...
                <div role="tabpanel" data-agile-panel="{{ $tab }}" {{ $ti === 0 ? '' : 'hidden' }}>
                    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                        @foreach ($processData[$tab] as $item)
                            <article
                                class="flex min-h-full flex-col gap-3 rounded-[20px] border border-[rgba(42,77,251,0.08)] bg-white p-[22px] shadow-[0_18px_40px_rgba(36,36,84,0.06)] transition hover:-translate-y-1 hover:shadow-[0_22px_48px_rgba(36,36,84,0.1)]">
                                <span
                                    class="inline-flex h-12 w-12 items-center justify-center rounded-[14px] bg-[#EEF1FF]"><img
                                        src="{{ $item['icon'] ?? asset('assets/icons/agile-icon-1.svg') }}"
                                        alt="{{ ($item['title'] ?? 'Agile process') . ' icon for Suave Creators industry development' }}"
...
<symbol id="core-value-logistics" viewBox="0 0 24 24">
  ...
  <div class="core-values__inner section-inner">
    ...
    $coreValues = [
      ...
      ['IT', 'IT Solutions for Startups', 'Innovative solutions tailored to your business needs.', '/assets/media/it_solutions.webp', 'Innovative solutions tailored to your business needs.'],
      ['logistics', 'Logistics and Supply Chain', 'Streamlined warehouse, inventory & transport management.', '/assets/media/supply_chain.webp', 'Streamlined warehouse, inventory & transport management.'],
    ];
    @endphp
